...
export function WhatsAppSettings() {
  ...
  const loadSettings = async () => {
    try {
      ...
      setAutomationEnabled(data.settings?.automation_enabled !== false);
      const overview = await whatsappRequest("/automation");
      setWorkflow(overview.templates);
    } catch (err: any) {
    ...
    <div className="space-y-5 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 font-semibold mt-0.5">
            Twilio connection, webhooks, and approved message templates.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Twilio credentials are configured server-side only and are never
            shown here.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex items-center gap-2">
            {settings.connected ? (
              <Badge className="bg-emerald-50 text-emerald-700 border border-emerald-200 gap-1 font-bold">
                <CheckCircle2 className="h-3 w-3" /> Twilio Connected
              </Badge>
            ) : (
              <Badge className="bg-rose-50 text-rose-700 border border-rose-200 gap-1 font-bold">
                <XCircle className="h-3 w-3" /> Twilio Not Configured
              </Badge>
            )}
            {settings.webhookConfigured ? (
              <Badge className="bg-emerald-50 text-emerald-700 border border-emerald-200 gap-1 font-bold">
                <CheckCircle2 className="h-3 w-3" /> Webhook URL Set
              </Badge>
            ) : (
              <Badge className="bg-amber-50 text-amber-700 border border-amber-200 gap-1 font-bold">
                <XCircle className="h-3 w-3" /> PUBLIC_URL Not Set
              </Badge>
            )}
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <Input
                ...
                value={
                  ...
                  "Set PUBLIC_URL to generate this URL"
                }
              ...
              {settings.incomingWebhookUrl && (
                <Button
                  ...
                  size="icon"
                  onClick={() =>
                    copyToClipboard(settings.incomingWebhookUrl!, "Webhook URL")
                  }
                >
            ...
            <p className="text-[10px] text-slate-400 font-semibold mt-1">
              Set this as the "When a message comes in" webhook on your Twilio
              WhatsApp sender.
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-semibold mt-1">
              Applied automatically to every outbound message — used for
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            One question at a time. Answers are saved directly to the lead. Only
            active, approved Utility templates are sent.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            {workflow.map((t) => (
              ...
              >
                ...
                {t.rejection_reason && (
                  <p className="w-full text-rose-600">{t.rejection_reason}</p>
                )}
              </div>
            ))}
          </div>
          <div className="flex flex-wrap gap-2">
            <Button
              disabled={!settings.canManage || settingUp}
              onClick={async () => {
                setSettingUp(true);
                try {
                  const result = await whatsappRequest("/automation/setup", {});
                  ...
                  if (failures.length)
                    ...
                    toast.success(
                      "Workflow templates submitted. Approval is controlled by Meta.",
                    );
                  await loadSettings();
                } catch (e: any) {
            ...
            >
              {settingUp
                ? "Preparing templates…"
                : "Create / submit Utility templates"}
            </Button>
            <Button
              variant="outline"
              disabled={settingUp}
              onClick={async () => {
                try {
                  await whatsappRequest("/templates/sync", {});
                  await loadSettings();
                  toast.success("Approval statuses refreshed");
                } catch (e: any) {
                  toast.error(e.message);
                }
              }}
          ...
          <p className="text-xs text-slate-500">
            Pending, rejected or reclassified templates stop the corresponding
            automated reply and show an action-required notice in the inbox.
            Manual replies remain available during the customer's 24-hour
            window.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Applies to all agents. Only admins and sales managers can change
            these.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setSessionWindowHours(Number(e.target.value) || 24)
              }
              disabled
              className="mt-1 w-32"
            />
            <p className="text-[10px] text-slate-400 font-semibold mt-1">
              WhatsApp's platform limit is 24 hours — do not increase beyond
              that.
            </p>
          </div>
          {settings.canManage && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 font-semibold">
                  Create, sync, and track WhatsApp template approval status
                </p>
              </div>
            </div>
